...
int main(){
    ...
    int completed=0;for(int family=0;family<Families;family++)for(int seed=1;seed<=20;seed++){auto sim=hatched();sim.wins=20;sim.choose(family);sim.pet().hatch=30;sim.rng=seed;for(int i=0;i<10;i++)sim.care(3);sim.encounter(seed%4);int turns=0;while(sim.battle.active&&turns++<100){int a=sim.battle.hp<sim.battle.maxHp/3&&sim.battle.potion?3:sim.battle.focus>=2?1:0;assert(sim.move(a));assert(sim.battle.hp>=0&&sim.battle.enemyHp>=0);}assert(!sim.battle.active);assert(sim.wins==21||sim.losses==1);Game copy;assert(copy.load(sim.save()));completed++;}
